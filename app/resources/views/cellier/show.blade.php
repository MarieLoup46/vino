...
<div class="row">
    <div class="mx-5 mt-5">
        ...
        <form action="{{ route('cellier.update', $cellier->id) }}" method="POST" onsubmit="return confirm('Êtes-vous sûr de vouloir renommer votre cellier?');">
            @method('PUT')
            @csrf
        ...
        <form action="{{ route('cellier.destroy', $cellier->id) }}" method="POST" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer cette cellier?');">
            @method('DELETE')
            @csrf
